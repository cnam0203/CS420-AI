...
void setNextStep(Position &pos,Stack &stackPos, Block **block, int x, int y, bool &isUpdateStep) {
    ...
    if (compareStep(x+1, y, oldX, oldY) && block[x+1][y].isOk() && !block[x+1][y].isVisited()) {
        pos.setPos(x+1, y);
        Position newPos;
        newPos.setPos(x, y);
    ...
    if (compareStep(x, y-1, oldX, oldY) && block[x][y-1].isOk() && !block[x][y-1].isVisited()) {
        pos.setPos(x, y-1);
        Position newPos;
        newPos.setPos(x, y);
        stackPos.Push(newPos);
...
int main(int argc, const char * argv[]) {
    ...
    if (inputArr) {
        ...
        while (!(gold == numGold && isWumpDied) && step != 150 && !isAgentDied) {
            int x = pos.getX();
            int y = pos.getY();
            ...
            if (map[x][y] == 6) { // Agent in wumpus or pit position
                ...
                setNextStep(pos, stackPos, arrBlock, x, y, isUpdateStep);
                if (!isUpdateStep)
                    break;
                cout<<"At step "<<step+1<<": ("<<pos.getX()<<", "<<pos.getY()<<"), score: "<<gold<<endl;
            }
            step++;
        }
