...
void checkBlock(Block &block, int kind, int x, int y, bool &isWumpDied, Position &wumPos, bool &isDetected, string direct, int size) {
...
void predictBlock(Block **&arrBlock, int x, int y, int kind, bool &isWumpDied, Position &wumPos, int size) {
    bool isDetected = false;
    checkBlock(arrBlock[x][y - 1], kind, x, y - 1, isWumpDied, wumPos, isDetected, "<-", size); // Predict thằng bên trái


    checkBlock(arrBlock[x][y + 1], kind, x, y + 1, isWumpDied, wumPos, isDetected, "->", size); // Predict thằng bên phải
    if (isDetected) { // Nếu như thằng Wumpus đã detect được nằm trong block này, thì những block predicted trước đó được xem xét lại,
        checkBlock(arrBlock[x][y - 1], kind, x, y - 1, isWumpDied, wumPos, isDetected, "<-", size);
    }


    checkBlock(arrBlock[x - 1][y], kind, x - 1, y, isWumpDied, wumPos, isDetected, "|^", size); // Predict thằng phía trên
    ...
    checkBlock(arrBlock[x + 1][y], kind, x + 1, y, isWumpDied, wumPos, isDetected, "|v", size); // Predict thằng phía dưới
    if (isDetected) { // Tương tự
        checkBlock(arrBlock[x][y - 1], kind, x, y - 1, isWumpDied, wumPos, isDetected, "<-", size);
        checkBlock(arrBlock[x][y + 1], kind, x, y + 1, isWumpDied, wumPos, isDetected, "->", size);
        checkBlock(arrBlock[x + 1][y], kind, x + 1, y, isWumpDied, wumPos, isDetected, "|v", size);
    }
}
    ...
    if (inputArr) {
        ...
        while ((step+i)!= 150 && !isAgentDied) {
            ...
            if (map[x][y] == 6) { // Agent in wumpus or pit position
                ...
                predictBlock(arrBlock, x, y, map[x][y], isWumpDied, wumPos, size);
