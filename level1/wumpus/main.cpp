...
void checkBlock(Block &block, int kind, int x, int y, string direct, int size, int &score, int& shot_x, int& shot_y, ofstream &fout) {
    if (x==3 && y ==3)
        cout<<block.isOk()<<endl;
                    ...
                    if (block.isPredicted()) {  // Nếu block đã được predict
                        if (!block.isWumpDead()) { // Nếu có wumpus chưa die
                            if (block.isWump()) { // Nếu block trước đó đã được predict là có wumpus => giết
                                fout << "Shoot a wump at " << y+1 << "," << x+1 << " " << direct << endl;
                                score = score - 100;
                                shot_x = x;
                                shot_y = y;
                                block.setShot();
...
void predictBlock(Block **&arrBlock, int x, int y, int kind, int size, int &score, int &shot_x, int &shot_y, ofstream &fout, int& cur) {
    
    if (cur == 1) {
        checkBlock(arrBlock[x][y - 1], kind, x, y - 1, "<-", size, score, shot_x, shot_y, fout); // Predict thằng bên trái
        cur++;
    }
        if (shot_x == -1 && shot_y == -1)
        {
            if (cur == 2) {
                checkBlock(arrBlock[x][y + 1], kind, x, y + 1, "->", size, score, shot_x, shot_y, fout); // Predict thằng bên phải
                cur++;
            }
            if (shot_x == -1 && shot_y == -1)
            {
                if (cur == 3) {
                    checkBlock(arrBlock[x - 1][y], kind, x - 1, y, "|v", size, score, shot_x, shot_y, fout); // Predict thằng phía trên
                    cur++;
                }
                if (shot_x == -1 && shot_y == -1)
                {
                    if (cur == 4) {
                        checkBlock(arrBlock[x + 1][y], kind, x + 1, y, "|^", size, score, shot_x, shot_y, fout); // Predict thằng phía dưới
                        cur++;
                    }
                }
            }
...
//        int numGold = getNumGold(map, size);
        ...
        while ((step+i)!= 150 && !isAgentDied) {
            ...
            if (map[x][y] == 6) { // Agent in wumpus or pit position
                ...
                predictBlock(arrBlock, x, y, map[x][y], size, gold, shot_x, shot_y, fout, cur);
        ...
        cout << step << "Total score: " << gold << endl;

        fout<<"Total Score: "<<gold<<endl;
        fout.close();
